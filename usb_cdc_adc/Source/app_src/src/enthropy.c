...
void pushToRandomArry(double dataEnthropy, uint8_t* data, double minEnthropy, uint8_t validBits)
{
	if (dataEnthropy >= minEnthropy && (DATA_SIZE - arryPos >= DATA_SIZE_4))
	{
		memcpy(randomArry, data, DATA_SIZE_4);
		arryPos += DATA_SIZE_4;
	}
}
...
uint8_t* fullFillRandomArry(double minEnthropy)
{
	...
	pushToRandomArry(eTemp, temp, minEnthropy);
